...
class PlayerHub(pygame.sprite.Sprite):
    def __init__(self, x, y):
        ...
        self.animation_speed = 150 # milisegundos por fotograma
    ...
    def cargar_idle(self, image_path):
        """Carga y escala la imagen estática y la devuelve en una lista."""
        img = pygame.image.load(image_path).convert_alpha()
        # Ajusta el tamaño de la imagen estática si es necesario
        scaled_img = pygame.transform.scale(img, (PLAYER_FRAME_WIDTH, PLAYER_FRAME_HEIGHT))
        return [scaled_img]

    def cargar_animacion(self, spritesheet_path):
        """Carga una animación desde una hoja de sprites."""
        spritesheet = SpriteSheet(spritesheet_path)
        frames = []
        # Asumiendo que las hojas de sprites son verticales
