...
class ArcadeState(BaseState):
    def __init__(self):
        ...
        self.machines = {
            ...
            "ASTEROIDS": pygame.Rect(720, 280, 125, 300),
            "CREDITS": pygame.Rect(1000, 300, 200, 150),
            "SLOTS": pygame.Rect(200, 280, 125, 300) 
        }

        self.game_modes = {
            "PONG": ["Jugador vs CPU", "Jugador vs Jugador"],
            "SF": ["Classic"],
            "ASTEROIDS": ["Classic"]
        }
        ...
        if event.type == pygame.KEYDOWN:
            if self.maquina_activa is not None:
                ...
                if event.key == pygame.K_e and self.interaction_prompt_text is not None:
                    ...
                    elif machine in self.game_modes:
                        self.persistent['dinero_total'] = self.dinero_total
                        self.persistent['fichas'] = self.fichas
                        self.persistent['game_title'] = machine
                        self.persistent['game_modes'] = self.game_modes[machine]
                        self.persistent['high_scores'] = save_manager.load_high_scores(machine)
                        self.persistent['arcade_surface'] = pygame.display.get_surface().copy()
                        self.next_state = "PRE_GAME_MENU"
                        self.done = True
